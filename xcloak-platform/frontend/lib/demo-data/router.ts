...
export function demoRoute(
  ...
  // ── Script Runner Enterprise — mutation stubs ───────────────────────────────
  if (method === 'POST' && p === '/sr/scripts')                                   return ok({ id: Date.now() % 1000 + 100, script_id: `SCR-0001-${Date.now()%9999}` });
  if (method === 'PATCH' && /^\/sr\/scripts\/\d+$/.test(p))                      return ok({ ok: true });
  if (method === 'DELETE' && /^\/sr\/scripts\/\d+$/.test(p))                     return ok({ ok: true });
  if (method === 'POST' && p === '/sr/execute')                                   return ok({ execution_id: `EXEC-0001-${Date.now()%999999}`, approval_required: false, status: 'running' });
  if (method === 'POST' && p === '/sr/ai')                                        return ok({ action: 'generate', result: '{"script":"#!/bin/bash\\n# Collect system health metrics\\n\\nSET_DATE=$(date +\\\"%Y%m%d_%H%M%S\\\")\\nOUTPUT_FILE=\\"/tmp/health_${SET_DATE}.txt\\\\"\\n\\necho \\\"=== System Health Report: $(hostname) ===\\\" > $OUTPUT_FILE\\necho \\\"Date: $(date)\\\" >> $OUTPUT_FILE\\necho \\\"\\\\nCPU Usage:\\\" >> $OUTPUT_FILE\\ntop -bn1 | grep \\\\"Cpu(s)\\\\" >> $OUTPUT_FILE\\necho \\\"\\\\nMemory Usage:\\\" >> $OUTPUT_FILE\\nfree -h >> $OUTPUT_FILE\\necho \\\"\\\\nDisk Usage:\\\" >> $OUTPUT_FILE\\ndf -h >> $OUTPUT_FILE\\necho \\\"\\\\nTop Processes:\\\" >> $OUTPUT_FILE\\nps aux --sort=-%cpu | head -10 >> $OUTPUT_FILE\\n\\ncat $OUTPUT_FILE\\nexit 0","description":"Collects CPU, memory, disk, and top process metrics from a Linux endpoint and outputs a structured health report.","parameters":[{"name":"output_dir","type":"string","description":"Directory for output file","required":false,"default":"/tmp"},{"name":"include_network","type":"boolean","description":"Include network statistics","required":false,"default":false}],"warnings":["Requires read access to /proc","Output file written to /tmp — ensure cleanup","top command behavior may vary across distributions"]}' });
  if (method === 'POST' && p === '/sr/schedules')                                 return ok({ id: Date.now() % 1000 + 100 });
  if (method === 'PATCH' && /^\/sr\/schedules\/\d+$/.test(p))                    return ok({ ok: true });
  if (method === 'DELETE' && /^\/sr\/schedules\/\d+$/.test(p))                   return ok({ ok: true });
  if (method === 'POST' && /^\/sr\/approvals\/\d+\/decide$/.test(p))             return ok({ ok: true, decision: 'approve', execution_id: `EXEC-0001-${Date.now()%999999}` });
  if (method === 'POST' && p === '/sr/report')                                    return ok({ title: 'Script Execution Report', report_type: 'execution', generated_at: new Date().toISOString(), generated_by: 'analyst@xcloak.io', classification: 'CONFIDENTIAL', executive_summary: 'During the reporting period, 847 script executions were recorded with a 96.2% success rate. 815 executions completed successfully across 124 unique endpoints, saving an estimated 340+ analyst-hours compared to manual operations. 32 executions failed, primarily in the network-scan and patch-deploy categories.', key_metrics: { total_executions: 847, success_rate: '96.2%', failed: 32, total_scripts: 38, scheduled_jobs: 14 }, recommendations: ['Enable script signing for all production scripts', 'Implement approval workflow for all root-level executions', 'Schedule regular audit reviews of high-frequency scripts', 'Enable secret vault integration for credential management'] });

  // ── Quarantine Enterprise — mutation stubs ───────────────────────────────
  if (method === 'POST' && p === '/qe/items')                            return ok({ id: Date.now() % 1000 + 100, quarantine_id: `QE-0001-${Date.now()%9999}`, ok: true, approval_required: false });
  if (method === 'POST' && /^\/qe\/items\/\d+\/action$/.test(p))        return ok({ ok: true, action: 'release' });
  if (method === 'POST' && /^\/qe\/items\/\d+\/approve$/.test(p))       return ok({ ok: true, decision: 'approve' });
  if (method === 'POST' && /^\/qe\/items\/\d+\/evidence$/.test(p))      return ok({ ok: true, evidence_types: ['processes','network_connections','event_logs','file_metadata','registry_changes'] });
  if (method === 'POST' && p === '/qe/ai')                              return ok({ ai_analysis: '{"threat_summary":"Endpoint WIN-WORKST-047 was quarantined after EDR detected Cobalt Strike beacon activity. Process hollowing detected in svchost.exe with C2 communication to 185.220.101.47:443.","root_cause":"Initial access via spear-phishing email attachment (macro-enabled XLSM). Execution chain: winword.exe → cmd.exe → powershell.exe → svchost.exe (hollowed).","recommended_actions":["Collect full memory dump immediately","Block C2 IP 185.220.101.47 at perimeter firewall","Preserve disk image before releasing","Analyze parent process chain for persistence mechanisms","Check for lateral movement from this endpoint"],"estimated_business_impact":"Medium-High — endpoint is a developer workstation with access to source code repositories. If credentials were exfiltrated, code signing and repo integrity may be compromised.","similar_historical_cases":["INC-2024-0341 — identical Cobalt Strike beacon profile (same C2 IP). Resolved via reimaging.","INC-2024-0198 — macro phishing campaign targeting dev team. 3 endpoints compromised."],"release_recommendation":"Do NOT release until: (1) memory dump analyzed for exfiltrated credentials, (2) C2 infrastructure blocked, (3) endpoint reimaged or verified clean by EDR, (4) password reset for all accounts accessed from this machine."}' });
  if (method === 'POST' && p === '/qe/report')                          return ok({ title: 'Quarantine Activity Report', generated_at: new Date().toISOString(), generated_by: 'analyst@xcloak.io', classification: 'CONFIDENTIAL', executive_summary: 'During the reporting period, 12 assets were quarantined across endpoint, file, user, and network categories. 9 assets were successfully released after threat verification. Endpoint quarantines represented the highest volume (5), with 2 requiring executive approval due to critical severity classification. Average quarantine duration was 4.2 hours, with a 92% successful release rate.', key_metrics: { total_quarantined: 12, active: 3, released: 9, avg_duration_hours: 4.2, release_success_rate: '92%', pending_approval: 1 }, by_type_summary: { endpoint: 5, file: 3, user: 2, network: 1, process: 1 }, top_detections: ['Cobalt Strike Beacon (3)', 'Credential Dumping (2)', 'Malicious Macro Execution (2)', 'Lateral Movement (2)'], recommendations: ['Implement automated release for file quarantines after hash verification', 'Enable manager approval for all user account quarantines', 'Ensure evidence collection triggers within 5 minutes of quarantine', 'Review repeat-offender assets for persistent compromise'] });
  ...
  // ── Script Runner Enterprise — GET fixtures ─────────────────────────────
  if (p === '/sr/dashboard') return ok({ total_scripts: 38, active_scripts: 31, running_jobs: 2, scheduled_jobs: 14, successful_executions: 815, failed_executions: 32, avg_execution_time: 4820, pending_approvals: 3, managed_endpoints: 124 });
  if (p === '/sr/scripts') return ok([
    { id: 1, script_id: 'SCR-0001-11111', name: 'System Health Check', description: 'Collects CPU, memory, disk, and process metrics from endpoints.', category: 'monitoring', language: 'bash', version: '2.1.0', author: 'alice@corp.com', status: 'active', tags: '["monitoring","health","linux"]', requires_approval: false, last_modified: new Date(Date.now()-86400000).toISOString(), created_at: new Date(Date.now()-2592000000).toISOString() },
    { id: 2, script_id: 'SCR-0001-22222', name: 'Malware Artifact Cleanup', description: 'Removes known malware artifacts, registry keys, and persistence mechanisms.', category: 'remediation', language: 'powershell', version: '1.4.2', author: 'bob@corp.com', status: 'active', tags: '["remediation","malware","windows"]', requires_approval: true, last_modified: new Date(Date.now()-172800000).toISOString(), created_at: new Date(Date.now()-5184000000).toISOString() },
    { id: 3, script_id: 'SCR-0001-33333', name: 'Network Port Scanner', description: 'Scans specified port ranges and reports open services on target hosts.', category: 'discovery', language: 'python', version: '3.0.1', author: 'carol@corp.com', status: 'active', tags: '["network","discovery","ports"]', requires_approval: false, last_modified: new Date(Date.now()-259200000).toISOString(), created_at: new Date(Date.now()-7776000000).toISOString() },
    { id: 4, script_id: 'SCR-0001-44444', name: 'Log Collection & Forward', description: 'Collects security event logs, compresses, and forwards to SIEM ingestion endpoint.', category: 'logging', language: 'powershell', version: '1.2.0', author: 'alice@corp.com', status: 'active', tags: '["logs","siem","collection"]', requires_approval: false, last_modified: new Date(Date.now()-345600000).toISOString(), created_at: new Date(Date.now()-10368000000).toISOString() },
    { id: 5, script_id: 'SCR-0001-55555', name: 'Patch Deployment', description: 'Downloads and applies security patches. Includes rollback capability.', category: 'patch', language: 'powershell', version: '4.0.0', author: 'dave@corp.com', status: 'active', tags: '["patch","windows","update"]', requires_approval: true, last_modified: new Date(Date.now()-432000000).toISOString(), created_at: new Date(Date.now()-12960000000).toISOString() },
    { id: 6, script_id: 'SCR-0001-66666', name: 'Process Memory Dump', description: 'Dumps memory of a specified process for forensic analysis.', category: 'forensics', language: 'python', version: '2.0.0', author: 'alice@corp.com', status: 'active', tags: '["forensics","memory","dfir"]', requires_approval: true, last_modified: new Date(Date.now()-518400000).toISOString(), created_at: new Date(Date.now()-15552000000).toISOString() },
    { id: 7, script_id: 'SCR-0001-77777', name: 'Account Lockout Check', description: 'Checks Active Directory for locked accounts and reports with lockout reason.', category: 'identity', language: 'powershell', version: '1.0.3', author: 'eve@corp.com', status: 'active', tags: '["ad","identity","lockout"]', requires_approval: false, last_modified: new Date(Date.now()-604800000).toISOString(), created_at: new Date(Date.now()-18144000000).toISOString() },
    { id: 8, script_id: 'SCR-0001-88888', name: 'Container Security Scan', description: 'Scans Docker containers for CVEs, misconfigurations, and malicious images.', category: 'security', language: 'bash', version: '1.1.0', author: 'frank@corp.com', status: 'deprecated', tags: '["container","docker","security"]', requires_approval: false, last_modified: new Date(Date.now()-691200000).toISOString(), created_at: new Date(Date.now()-20736000000).toISOString() },
  ]);
  if (/^\/sr\/scripts\/\d+$/.test(p)) return ok({ id: 1, script_id: 'SCR-0001-11111', name: 'System Health Check', description: 'Collects CPU, memory, disk, and process metrics from endpoints.', category: 'monitoring', language: 'bash', version: '2.1.0', author: 'alice@corp.com', status: 'active', content: '#!/bin/bash\n# System Health Check Script v2.1.0\n# Collects key system metrics for security monitoring\n\nset -euo pipefail\n\nHOSTNAME=$(hostname)\nDATE=$(date "+%Y-%m-%d %H:%M:%S")\n\necho "=== XCloak System Health Check ==="\necho "Host: $HOSTNAME"\necho "Date: $DATE"\necho ""\necho "--- CPU ---"\ntop -bn1 | grep "Cpu(s)" | awk \'{print "Usage:", $2+$4"%"}\'\necho ""\necho "--- Memory ---"\nfree -h | awk \'/Mem:/ {printf "Total: %s, Used: %s, Free: %s\\n", $2, $3, $4}\'\necho ""\necho "--- Disk ---"\ndf -h | grep -v tmpfs\necho ""\necho "--- Top Processes by CPU ---"\nps aux --sort=-%cpu | head -6 | tail -5\necho ""\necho "--- Active Network Connections ---"\nss -tnp | head -20\n\nexit 0', tags: '["monitoring","health","linux"]', parameters: '[{"name":"output_dir","type":"string","description":"Directory for output file","required":false,"default":"/tmp"},{"name":"include_network","type":"boolean","description":"Include network statistics","required":false,"default":true}]', requires_approval: false, is_signed: true, last_modified: new Date(Date.now()-86400000).toISOString(), created_at: new Date(Date.now()-2592000000).toISOString() });
  if (p === '/sr/executions') return ok([
    { id: 1, execution_id: 'EXEC-0001-847291', script_id: 'SCR-0001-11111', script_name: 'System Health Check', target: 'WIN-WORKST-047', target_count: 1, status: 'success', exit_code: 0, execution_time: 3240, trigger_source: 'manual', run_as: 'system', executed_by: 'alice@corp.com', started_at: new Date(Date.now()-3600000).toISOString(), completed_at: new Date(Date.now()-3596760).toISOString() },
    { id: 2, execution_id: 'EXEC-0001-831047', script_id: 'SCR-0001-44444', script_name: 'Log Collection & Forward', target: 'Asset Group: Servers', target_count: 12, status: 'success', exit_code: 0, execution_time: 28400, trigger_source: 'scheduled', run_as: 'system', executed_by: 'scheduler', started_at: new Date(Date.now()-7200000).toISOString(), completed_at: new Date(Date.now()-7171600).toISOString() },
    { id: 3, execution_id: 'EXEC-0001-819203', script_id: 'SCR-0001-33333', script_name: 'Network Port Scanner', target: '10.0.1.0/24', target_count: 1, status: 'running', exit_code: null, execution_time: null, trigger_source: 'manual', run_as: 'system', executed_by: 'bob@corp.com', started_at: new Date(Date.now()-120000).toISOString(), completed_at: null },
    { id: 4, execution_id: 'EXEC-0001-807482', script_id: 'SCR-0001-22222', script_name: 'Malware Artifact Cleanup', target: 'WIN-SRV-DC01', target_count: 1, status: 'failed', exit_code: 1, execution_time: 15200, trigger_source: 'incident', run_as: 'administrator', executed_by: 'carol@corp.com', started_at: new Date(Date.now()-14400000).toISOString(), completed_at: new Date(Date.now()-14384800).toISOString() },
    { id: 5, execution_id: 'EXEC-0001-795019', script_id: 'SCR-0001-55555', script_name: 'Patch Deployment', target: 'Department: Finance', target_count: 8, status: 'success', exit_code: 0, execution_time: 142000, trigger_source: 'playbook', run_as: 'system', executed_by: 'playbook-engine', started_at: new Date(Date.now()-86400000).toISOString(), completed_at: new Date(Date.now()-86258000).toISOString() },
    { id: 6, execution_id: 'EXEC-0001-783201', script_id: 'SCR-0001-66666', script_name: 'Process Memory Dump', target: 'WIN-WORKST-047', target_count: 1, status: 'success', exit_code: 0, execution_time: 8900, trigger_source: 'alert', run_as: 'administrator', executed_by: 'alice@corp.com', started_at: new Date(Date.now()-172800000).toISOString(), completed_at: new Date(Date.now()-172791100).toISOString() },
  ]);
  if (/^\/sr\/executions\/\d+$/.test(p)) return ok({ id: 1, execution_id: 'EXEC-0001-847291', script_id: 'SCR-0001-11111', script_name: 'System Health Check', target: 'WIN-WORKST-047', target_count: 1, status: 'success', exit_code: 0, stdout: '=== XCloak System Health Check ===\nHost: WIN-WORKST-047\nDate: 2026-07-17 09:41:22\n\n--- CPU ---\nUsage: 12.4%\n\n--- Memory ---\nTotal: 31Gi, Used: 14Gi, Free: 17Gi\n\n--- Disk ---\nFilesystem      Size  Used Avail Use%\n/dev/sda1        240G   89G  151G  37%\n\n--- Top Processes by CPU ---\n  PID %CPU %MEM COMMAND\n 4892  8.2  2.1 svchost.exe\n 1234  4.1  0.8 chrome.exe\n 2048  2.3  0.4 defender.exe\n\n--- Active Network Connections ---\nState  Recv-Q Send-Q Local Address  Foreign Address\nESTAB  0      0  10.0.1.47:445  10.0.1.12:49821', stderr: '', execution_time: 3240, trigger_source: 'manual', run_as: 'system', parameters: '{}', executed_by: 'alice@corp.com', started_at: new Date(Date.now()-3600000).toISOString(), completed_at: new Date(Date.now()-3596760).toISOString() });
  if (p === '/sr/schedules') return ok([
    { id: 1, name: 'Daily Health Check — All Servers', script_id: 'SCR-0001-11111', script_name: 'System Health Check', schedule_type: 'daily', cron_expr: '0 6 * * *', target: 'Asset Group: Servers', run_as: 'system', enabled: true, last_run: new Date(Date.now()-86400000).toISOString(), next_run: new Date(Date.now()+21600000).toISOString(), created_by: 'alice@corp.com', created_at: new Date(Date.now()-7776000000).toISOString() },
    { id: 2, name: 'Hourly Log Collection', script_id: 'SCR-0001-44444', script_name: 'Log Collection & Forward', schedule_type: 'cron', cron_expr: '0 * * * *', target: 'Asset Group: All Endpoints', run_as: 'system', enabled: true, last_run: new Date(Date.now()-3600000).toISOString(), next_run: new Date(Date.now()+1800000).toISOString(), created_by: 'scheduler', created_at: new Date(Date.now()-15552000000).toISOString() },
    { id: 3, name: 'Weekly Patch Check', script_id: 'SCR-0001-55555', script_name: 'Patch Deployment', schedule_type: 'weekly', cron_expr: '0 2 * * 0', target: 'Department: All', run_as: 'system', enabled: true, last_run: new Date(Date.now()-604800000).toISOString(), next_run: new Date(Date.now()+259200000).toISOString(), created_by: 'dave@corp.com', created_at: new Date(Date.now()-5184000000).toISOString() },
    { id: 4, name: 'Monthly AD Account Audit', script_id: 'SCR-0001-77777', script_name: 'Account Lockout Check', schedule_type: 'monthly', cron_expr: '0 3 1 * *', target: 'Domain Controllers', run_as: 'system', enabled: false, last_run: new Date(Date.now()-2592000000).toISOString(), next_run: null, created_by: 'eve@corp.com', created_at: new Date(Date.now()-18144000000).toISOString() },
  ]);
  if (p === '/sr/approvals') return ok([
    { id: 1, execution_id: 'EXEC-0001-PEND1', script_id: 'SCR-0001-22222', script_name: 'Malware Artifact Cleanup', target: 'WIN-SRV-DC01', run_as: 'administrator', requested_by: 'alice@corp.com', reason: 'Root execution on Domain Controller requires approval', decision: 'pending', decided_by: null, decided_at: null, created_at: new Date(Date.now()-900000).toISOString() },
    { id: 2, execution_id: 'EXEC-0001-PEND2', script_id: 'SCR-0001-66666', script_name: 'Process Memory Dump', target: 'PROD-SRV-ERP01', run_as: 'root', requested_by: 'bob@corp.com', reason: 'Memory dump on production ERP server — approval required per policy', decision: 'pending', decided_by: null, decided_at: null, created_at: new Date(Date.now()-1800000).toISOString() },
    { id: 3, execution_id: 'EXEC-0001-PEND3', script_id: 'SCR-0001-55555', script_name: 'Patch Deployment', target: 'Asset Group: Production Servers', run_as: 'system', requested_by: 'carol@corp.com', reason: 'Bulk patch deployment to 24 production servers requires senior approval', decision: 'pending', decided_by: null, decided_at: null, created_at: new Date(Date.now()-3600000).toISOString() },
  ]);
  if (p === '/sr/analytics') return ok({ total_executions: 847, successful: 815, failed: 32, success_rate: 96, avg_execution_time: 4820, total_scripts: 38, active_scripts: 31, automation_time_saved_hours: 340, most_executed: [{ name: 'System Health Check', count: 312 }, { name: 'Log Collection & Forward', count: 248 }, { name: 'Network Port Scanner', count: 87 }, { name: 'Patch Deployment', count: 63 }, { name: 'Account Lockout Check', count: 41 }], by_category: [{ category: 'monitoring', count: 14 }, { category: 'remediation', count: 8 }, { category: 'forensics', count: 6 }, { category: 'patch', count: 5 }, { category: 'logging', count: 4 }, { category: 'discovery', count: 3 }], execution_trend: [{ date: '2026-07-11', count: 98 }, { date: '2026-07-12', count: 134 }, { date: '2026-07-13', count: 112 }, { date: '2026-07-14', count: 89 }, { date: '2026-07-15', count: 156 }, { date: '2026-07-16', count: 143 }, { date: '2026-07-17', count: 115 }] });
  if (p === '/sr/audit') return ok([
    { id: 1, action: 'executed', script_id: 'SCR-0001-11111', script_name: 'System Health Check', actor: 'alice@corp.com', details: 'Target: WIN-WORKST-047, RunAs: system', created_at: new Date(Date.now()-3600000).toISOString() },
    { id: 2, action: 'approval_required', script_id: 'SCR-0001-22222', script_name: 'Malware Artifact Cleanup', actor: 'alice@corp.com', details: 'Target: WIN-SRV-DC01, RunAs: administrator', created_at: new Date(Date.now()-7200000).toISOString() },
    { id: 3, action: 'created', script_id: 'SCR-0001-33333', script_name: 'Network Port Scanner', actor: 'carol@corp.com', details: 'Language: python, Category: discovery', created_at: new Date(Date.now()-86400000).toISOString() },
    { id: 4, action: 'modified', script_id: 'SCR-0001-44444', script_name: 'Log Collection & Forward', actor: 'alice@corp.com', details: 'Script content updated', created_at: new Date(Date.now()-172800000).toISOString() },
    { id: 5, action: 'approved', script_id: 'SCR-0001-55555', script_name: 'Patch Deployment', actor: 'manager@corp.com', details: 'Execution EXEC-0001-795019 approved and started', created_at: new Date(Date.now()-259200000).toISOString() },
    { id: 6, action: 'scheduled', script_id: 'SCR-0001-11111', script_name: 'System Health Check', actor: 'alice@corp.com', details: 'Schedule: daily, Target: Asset Group: Servers', created_at: new Date(Date.now()-345600000).toISOString() },
    { id: 7, action: 'rejected', script_id: 'SCR-0001-66666', script_name: 'Process Memory Dump', actor: 'manager@corp.com', details: 'Execution EXEC-0001-783202 rejected — requires CISO approval', created_at: new Date(Date.now()-432000000).toISOString() },
    { id: 8, action: 'deleted', script_id: 'SCR-0001-00099', script_name: 'Legacy Cleanup Script', actor: 'dave@corp.com', details: 'Script deleted from library', created_at: new Date(Date.now()-518400000).toISOString() },
  ]);

  // ── Deception Enterprise ──────────────────────────────────────────────────
  // ── Quarantine Enterprise — GET fixtures ────────────────────────────────
  if (p === '/qe/dashboard') return ok({ quarantined_endpoints: 3, quarantined_files: 4, quarantined_processes: 2, quarantined_users: 1, quarantined_emails: 8, quarantined_network_connections: 2, active_quarantine_sessions: 12, released_assets: 47, pending_approvals: 1, expiring_soon: 2 });
  if (p === '/qe/queue') return ok([
    { id: 1, quarantine_id: 'QE-0001-84231', asset_name: 'WIN-WORKST-047', asset_type: 'endpoint', severity: 'critical', risk_score: 94, status: 'active', owner: 'alice@corp.com', source_detection: 'Cobalt Strike Beacon', incident_id: 'INC-2026-0341', case_id: 'CASE-0089', quarantine_type: 'full_network_isolation', quarantine_reason: 'Active C2 beacon detected. Process hollowing in svchost.exe.', approval_status: 'not_required', evidence_collected: true, expires_at: null, created_at: new Date(Date.now()-7200000).toISOString() },
    { id: 2, quarantine_id: 'QE-0001-71049', asset_name: 'malware_dropper.exe', asset_type: 'file', severity: 'high', risk_score: 88, status: 'active', owner: 'bob@corp.com', source_detection: 'Malicious PE Execution', incident_id: 'INC-2026-0338', case_id: '', quarantine_type: 'move_to_secure_storage', quarantine_reason: 'Suspicious PE with packed section and C2 hardcoded.', approval_status: 'not_required', evidence_collected: true, expires_at: null, created_at: new Date(Date.now()-14400000).toISOString() },
    { id: 3, quarantine_id: 'QE-0001-59812', asset_name: 'svchost.exe (PID 4892)', asset_type: 'process', severity: 'critical', risk_score: 91, status: 'active', owner: 'alice@corp.com', source_detection: 'Process Hollowing Detected', incident_id: 'INC-2026-0341', case_id: 'CASE-0089', quarantine_type: 'suspend_process', quarantine_reason: 'Process hollowing with shellcode injection detected.', approval_status: 'approved', evidence_collected: true, expires_at: null, created_at: new Date(Date.now()-7000000).toISOString() },
    { id: 4, quarantine_id: 'QE-0001-44720', asset_name: 'jsmith@corp.com', asset_type: 'user', severity: 'high', risk_score: 82, status: 'active', owner: 'carol@corp.com', source_detection: 'Credential Dumping — LSASS', incident_id: 'INC-2026-0339', case_id: '', quarantine_type: 'disable_account', quarantine_reason: 'Account credentials potentially exfiltrated via LSASS dump.', approval_status: 'pending', evidence_collected: false, expires_at: null, created_at: new Date(Date.now()-3600000).toISOString() },
    { id: 5, quarantine_id: 'QE-0001-39201', asset_name: 'phishing@evil.ru', asset_type: 'email', severity: 'high', risk_score: 79, status: 'active', owner: 'bob@corp.com', source_detection: 'BEC Phishing Campaign', incident_id: '', case_id: '', quarantine_type: 'move_to_quarantine_mailbox', quarantine_reason: 'Mass phishing campaign. 47 emails delivered before detection.', approval_status: 'not_required', evidence_collected: true, expires_at: new Date(Date.now()+86400000).toISOString(), created_at: new Date(Date.now()-21600000).toISOString() },
    { id: 6, quarantine_id: 'QE-0001-28840', asset_name: '185.220.101.47', asset_type: 'network', severity: 'critical', risk_score: 96, status: 'active', owner: 'alice@corp.com', source_detection: 'C2 Infrastructure — Tor Exit Node', incident_id: 'INC-2026-0341', case_id: 'CASE-0089', quarantine_type: 'block_ip', quarantine_reason: 'Known Tor exit node used as Cobalt Strike C2.', approval_status: 'not_required', evidence_collected: false, expires_at: null, created_at: new Date(Date.now()-7100000).toISOString() },
    { id: 7, quarantine_id: 'QE-0001-19933', asset_name: 'SRV-FILESVR-02', asset_type: 'endpoint', severity: 'high', risk_score: 77, status: 'released', owner: 'carol@corp.com', source_detection: 'Ransomware Precursor Activity', incident_id: 'INC-2026-0330', case_id: '', quarantine_type: 'management_network_only', quarantine_reason: 'VSS deletion and backup enumeration detected.', approval_status: 'not_required', evidence_collected: true, expires_at: null, created_at: new Date(Date.now()-172800000).toISOString() },
    { id: 8, quarantine_id: 'QE-0001-11127', asset_name: 'invoice_Q3.xlsm', asset_type: 'file', severity: 'medium', risk_score: 63, status: 'released', owner: 'bob@corp.com', source_detection: 'Malicious Macro Execution', incident_id: '', case_id: '', quarantine_type: 'encrypt_file', quarantine_reason: 'VBA macro with auto-open that spawned PowerShell.', approval_status: 'not_required', evidence_collected: true, expires_at: null, created_at: new Date(Date.now()-259200000).toISOString() },
  ]);
  if (/^\/qe\/items\/\d+$/.test(p)) return ok({ id: 1, quarantine_id: 'QE-0001-84231', asset_name: 'WIN-WORKST-047', asset_type: 'endpoint', severity: 'critical', risk_score: 94, status: 'active', owner: 'alice@corp.com', source_detection: 'Cobalt Strike Beacon', incident_id: 'INC-2026-0341', case_id: 'CASE-0089', quarantine_type: 'full_network_isolation', quarantine_reason: 'Active C2 beacon detected. Process hollowing in svchost.exe.', detection_rule: 'SIGMA-COBALT-BEACON-003', mitre_techniques: '["T1055 - Process Injection","T1071.001 - Web Protocols C2","T1059.001 - PowerShell","T1047 - WMI"]', related_alerts: '["ALT-2026-8821","ALT-2026-8830","ALT-2026-8847"]', business_impact: 'Developer workstation with source repo access. Potential code signing key exposure.', analyst_notes: 'Memory dump collected. C2 blocked at firewall. Awaiting full forensics before release.', approval_status: 'not_required', approved_by: '', evidence_collected: true, evidence_types: '["processes","network_connections","memory_dump","event_logs","registry_changes"]', release_type: '', expires_at: null, created_at: new Date(Date.now()-7200000).toISOString() });
  if (/^\/qe\/items\/\d+\/evidence$/.test(p)) return ok([
    { id: 1, evidence_type: 'processes', data: '{"count":142,"suspicious":["svchost.exe (PID 4892) — parent: powershell.exe","rundll32.exe — no module","cmd.exe — parent: winword.exe"]}', collected_at: new Date(Date.now()-6900000).toISOString() },
    { id: 2, evidence_type: 'network_connections', data: '{"count":28,"external":[{"dst":"185.220.101.47","port":443,"proto":"TCP","process":"svchost.exe","bytes_out":142800}],"internal":[{"dst":"10.0.1.88","port":445,"proto":"TCP","process":"svchost.exe"}]}', collected_at: new Date(Date.now()-6800000).toISOString() },
    { id: 3, evidence_type: 'memory_dump', data: '{"size_mb":4096,"shellcode_regions":2,"injected_dlls":["beacon.dll"],"strings_ioc":["185.220.101.47","pipe\\\\msagent_84"]}', collected_at: new Date(Date.now()-6700000).toISOString() },
    { id: 4, evidence_type: 'event_logs', data: '{"count":3847,"filtered":156,"key_events":[{"id":4688,"desc":"PowerShell spawned by winword.exe"},{"id":7045,"desc":"New service installed: svc_helper32"},{"id":4768,"desc":"Kerberos TGT request for jsmith@corp.com from this endpoint"}]}', collected_at: new Date(Date.now()-6600000).toISOString() },
    { id: 5, evidence_type: 'registry_changes', data: '{"count":23,"persistence":["HKCU\\\\Software\\\\Microsoft\\\\Windows\\\\CurrentVersion\\\\Run: svc_helper32","HKLM\\\\SYSTEM\\\\CurrentControlSet\\\\Services\\\\svc_helper32 — ImagePath: C:\\\\Windows\\\\Temp\\\\svc_helper32.exe"]}', collected_at: new Date(Date.now()-6500000).toISOString() },
  ]);
  if (p === '/qe/audit') return ok([
    { id: 1, item_id: 1, quarantine_id: 'QE-0001-84231', asset_name: 'WIN-WORKST-047', action: 'created', actor: 'alice@corp.com', details: 'Quarantine initiated for endpoint WIN-WORKST-047', created_at: new Date(Date.now()-7200000).toISOString() },
    { id: 2, item_id: 1, quarantine_id: 'QE-0001-84231', asset_name: 'WIN-WORKST-047', action: 'evidence_collected', actor: 'system', details: 'Automated evidence collection completed', created_at: new Date(Date.now()-6900000).toISOString() },
    { id: 3, item_id: 1, quarantine_id: 'QE-0001-84231', asset_name: 'WIN-WORKST-047', action: 'notes_updated', actor: 'alice@corp.com', details: 'Analyst notes updated', created_at: new Date(Date.now()-5400000).toISOString() },
    { id: 4, item_id: 4, quarantine_id: 'QE-0001-44720', asset_name: 'jsmith@corp.com', action: 'created', actor: 'carol@corp.com', details: 'Quarantine initiated for user jsmith@corp.com', created_at: new Date(Date.now()-3600000).toISOString() },
    { id: 5, item_id: 4, quarantine_id: 'QE-0001-44720', asset_name: 'jsmith@corp.com', action: 'approval_required', actor: 'system', details: 'High impact — awaiting SOC Lead approval', created_at: new Date(Date.now()-3580000).toISOString() },
    { id: 6, item_id: 7, quarantine_id: 'QE-0001-19933', asset_name: 'SRV-FILESVR-02', action: 'released', actor: 'carol@corp.com', details: 'Asset released. Threat verified removed. Endpoint health check passed.', created_at: new Date(Date.now()-86400000).toISOString() },
    { id: 7, item_id: 8, quarantine_id: 'QE-0001-11127', asset_name: 'invoice_Q3.xlsm', action: 'released', actor: 'bob@corp.com', details: 'File released after hash verification and sandbox analysis confirmed no persistence.', created_at: new Date(Date.now()-172800000).toISOString() },
  ]);
  if (p === '/qe/analytics') return ok({ total_quarantined: 59, active: 12, released: 47, pending_approval: 1, quarantine_trend: [{ date: '2026-07-11', count: 6 }, { date: '2026-07-12', count: 8 }, { date: '2026-07-13', count: 4 }, { date: '2026-07-14', count: 11 }, { date: '2026-07-15', count: 7 }, { date: '2026-07-16', count: 9 }, { date: '2026-07-17', count: 12 }], by_type: { endpoint: 5, file: 4, process: 2, user: 1, email: 8, network: 2 }, avg_duration_hours: 4.2, release_success_rate: 92, repeat_offenders: [{ asset: 'WIN-WORKST-022', quarantine_count: 3, last_at: new Date(Date.now()-86400000).toISOString() }, { asset: 'jdoe@corp.com', quarantine_count: 2, last_at: new Date(Date.now()-172800000).toISOString() }], top_detection_sources: [{ source: 'Cobalt Strike Beacon', count: 8 }, { source: 'Malicious Macro Execution', count: 6 }, { source: 'Credential Dumping — LSASS', count: 4 }, { source: 'BEC Phishing Campaign', count: 3 }, { source: 'Ransomware Precursor Activity', count: 2 }] });
