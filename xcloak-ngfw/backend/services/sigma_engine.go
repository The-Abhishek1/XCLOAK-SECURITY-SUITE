...
// ─────────────────────────────────────────────────────────────────────────────
// EvaluateCondition evaluates a boolean condition expression over a map of
// named selection results. Supports: and, or, not, parentheses.
//
// Examples:
//   "selection1"                          → value of selection1
//   "selection1 and selection2"           → both must be true
//   "selection1 or selection2"            → either true
//   "selection1 and not selection2"       → s1 true, s2 false
//   "(selection1 or selection2) and s3"   → parentheses respected
// ─────────────────────────────────────────────────────────────────────────────

func EvaluateCondition(condition string, results map[string]bool) bool {
	if condition == "" {
		// No condition — true if ANY selection matched.
	...
	val, _ := parseExpr(tokens, 0, results)
	return val
}
...
func tokenizeCondition(expr string) []string {
	...
	// Insert spaces around parentheses so they tokenize separately.
...
func parseExpr(tokens []string, pos int, results map[string]bool) (bool, int) {
	left, pos := parseAnd(tokens, pos, results)

	for pos < len(tokens) && tokens[pos] == "or" {
		pos++ // consume "or"
		right, newPos := parseAnd(tokens, pos, results)
		left = left || right
		pos = newPos
	}
	return left, pos
...
// parseAnd parses an AND-level expression.
func parseAnd(tokens []string, pos int, results map[string]bool) (bool, int) {
	left, pos := parseNot(tokens, pos, results)

	for pos < len(tokens) && tokens[pos] == "and" {
		pos++ // consume "and"
		right, newPos := parseNot(tokens, pos, results)
		left = left && right
		pos = newPos
	}
	return left, pos
...
// parseNot handles the NOT unary operator.
func parseNot(tokens []string, pos int, results map[string]bool) (bool, int) {
	if pos < len(tokens) && tokens[pos] == "not" {
		pos++ // consume "not"
		val, newPos := parsePrimary(tokens, pos, results)
		return !val, newPos
	}
	return parsePrimary(tokens, pos, results)
}

// parsePrimary handles parenthesized groups and selection names.
func parsePrimary(tokens []string, pos int, results map[string]bool) (bool, int) {
	if pos >= len(tokens) {
		return false, pos
	}
	...
	if tokens[pos] == "(" {
		pos++ // consume "("
		val, newPos := parseExpr(tokens, pos, results)
	...
	// Should be a selection name.
	name := tokens[pos]
	pos++

	// Support wildcards: "selection*" matches any selection starting with "selection".
